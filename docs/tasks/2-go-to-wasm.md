...
*Hacks*
* Is it possible to use separate heap region reserved for the GC allocations and another reserved for the allocator, somehow by offseting the `__heap_base`.
...
3. Add new target similar to Rust's `wasm32-unknown-unknown`.
  ...
  * [ ] need better absrtaction since host functions are versioned, so the runtime implementation will depened on third party API that might change in the future.
...
5. [ ] Implement bump allocator.

* [ ] fix the error when passing 0 offset to a Wasm function.
* [ ] in the vm helper, some struct fields are undefined

6. Add tests to check the correctness and the performance.
